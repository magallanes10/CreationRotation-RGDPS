    ...
    socket.setOnMessageCallback([this, callback](const ix::WebSocketMessagePtr& msg) {
        if (msg->type == ix::WebSocketMessageType::Error) {
            ...
            Loader::get()->queueInMainThread([this, errReason]() {
                FLAlertLayer::create("CR Error", fmt::format("There was an error while connecting to the server: {}", errReason), "OK")->show();
            });
            return;
            ...
            middleware([this, callback]() {
                this->on<ErrorPacket>([](ErrorPacket* packet) {
                    FLAlertLayer::create("CR Error", fmt::format("The Creation Rotation server sent an error: <cy>{}</c>", packet->error).c_str(), "OK")->show();
                });
                callback();
                for (auto packetFn : packetQueue) packetFn();
                packetQueue.clear();
                this->showDisconnectPopup = true;
                Loader::get()->queueInMainThread([]() {
                    Notification::create("Connection sucessful!", NotificationIcon::Success, 1.5f)->show();
                });
            });
            return;
            ...
            Loader::get()->queueInMainThread([this, reason]() {
                Notification::create("Disconnected from Creation Rotation", NotificationIcon::Error, 1.5f)->show();
                if (disconnectEventCb) disconnectEventCb(reason);
                if (this->showDisconnectPopup) {
                    geode::createQuickPopup("Disconnected", fmt::format("You have been disconnected from the Creation Rotation servers. Reason:\n\n{}", reason), "OK", "Close", [this, reason](auto, bool) {
